...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, dropout=0.5, tie_weights=False):
        ...
        if rnn_type in ['LSTM', 'GRU']:
            # global layers 
            self.rnn2 = getattr(nn, rnn_type)(ninp, int(nhid/2), nlayers, dropout=dropout)
            # local layer
            self.rnn1 = getattr(nn, rnn_type)(ninp, nhid, int(nlayers/2), bidirectional=False, dropout=dropout)
        else:
            try:
        ...
        self.decoder = nn.Linear(int(nhid*1.5), ntoken)
    ...
    def init_hidden(self, bsz):
        ...
        if self.rnn_type == 'LSTM':
            return (weight.new_zeros(self.nlayers, bsz, int(self.nhid/2)),
                    weight.new_zeros(self.nlayers, bsz, int(self.nhid/2)))
        else:
